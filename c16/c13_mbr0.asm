...
;-------------------------------------------------------------------------------      
         ...
         ;跳过0#号描述符的槽位，所以就跳过了ebx+0x00,0x04
         
         ;创建1#描述符，这是一个向上扩展的数据段，对应0~4GB的线性地址空间
         ...
         mov dword [ebx+0x1c],0x00cf9600    ;地址范围是7c00~6c00   
  ...
  flush:                                  
         ...
         ;以下加载系统核心程序 
         mov edi,core_base_address          ;内核文件的第一个双字dd就是内核的长度
 ...
 setup:
         ...
         call make_gdt_descriptor           ;101 0 00，0x28
         mov [esi+0x28],eax                 ;描述符是分成2个部分安装的
         mov [esi+0x2c],edx                 ;28处安装低32位，2c处安装高32位
...
make_gdt_descriptor:                     ;构造描述符
                                         ...
                                         ;      位置，其它没用到的位置0） 
                                         ;返回：EDX:EAX=完整的描述符
         mov edx,eax                     ;EAX一会在构造描述符的高32位时还要用
         shl eax,16                      ;用于构造描述符的低32位
         or ax,bx                        ;此时ax的16位全是0，就是复制bx给ax。描述符前32位(EAX)构造完毕
      
         and edx,0xffff0000              ;基地址在edx中有备份，清除基地址中无关的位
         rol edx,8                       ;循环左移指令将高16位分成2半：16-23、24-31
