         ...
         idt_linear_address    equ  0x1F000 ;ch15-中断：中断描述符表的线性地址
         core_lin_alloc_at     equ  0x80100000      ;ch16
                                            ;内核中可用于分配的起始线性地址
         core_lin_tcb_addr     equ  0x8001f800      ;ch16
                                            ;内核任务TCB的高端线性地址
...
load_relocate_program:                      ;加载并重定位用户程序
  ...
  .clsp:
         ...
         call sys_routine_seg_sel:task_alloc_memory ;ch16：分页：修改
  ...
  .b4:
         ...
         mov ebx,esi                        ;ch16：分页：修改
         call sys_routine_seg_sel:task_alloc_memory ;ch16：分页：修改
         mov [es:esi+0x2c],ecx              ;登记0特权级堆栈基地址到TCB        
         mov eax,ecx
         ...
         ;ch14：登记基本的TSS表格内容
         mov word [es:ecx+0],0              ;ch15:反向链=0
         ...
         mov dx,[es:esi+0x12]               ;ch15：登记任务的I/O位图偏移
         mov [es:ecx+102],dx                ;到TSS中
      
         mov word [es:ecx+100],0            ;T=0
         
         ;ch15：访问用户程序头部，获取数据填充TSS
         ...
         mov [es:ecx+80],dx                 ;到TSS
         
         mov edx,[es:edi+0x20]              ;ch16：修改：堆栈的高端线性地址
         mov [es:ecx+56],edx                ;填写TSS的ESP域
         
         mov dx,[es:edi+0x04]               ;登记程序数据段（DS）选择子
         mov word [es:ecx+84],dx            ;到TSS中。注意，它指向程序头部段
         
         mov word [es:ecx+72],0             ;到TSS中的ES=0
         
         mov word [es:ecx+88],0             ;到TSS中的FS=0
         
         mov word [es:ecx+92],0             ;到TSS中的GS=0
...
start:
         ...
         mov ecx,mem_0_4_gb_seg_sel          ;ch15：令ES指向4GB数据段
         mov es,ecx
  ...
  .b1:
         ...
         ;ch16:在页目录内创建指向页目录自己的目录项
         mov dword [es:ebx+4092],0x00020003
         
         ;ch16:在页目录内创建与线性地址0x00000000对应的目录项
         mov dword [es:ebx+0],0x00021003    ;写入目录项（页表的物理地址和属性）
         
         ;ch16:创建与上面那个目录项相对应的页表，初始化页表项
  ...
  .b3:                                      ;其余的页表项置为无效
         ...
         ;ch16:在页目录内创建与线性地址0x80000000对应的目录项
         ...
         ;mov dword [es:ebx+esi],0x00021003  ;写入目录项(页表的物理地址和属性)
                                             ;目录单元的线性地址为0xFFFFF200
         mov dword [es:0xfffff800],0x00021003   ;ch16分页：FFC是最后一个表项1023号表项的起始字节
                                            
         ;ch16:将GDT中的段描述符映射到线性地址0x80000000
         sgdt [pgdt]
  ...
  .g0:
         ...
         mov dword [es:ecx+0x46],core_lin_alloc_at  ;登记内核中可用于分配得起始线性地址     
         ;cli                                ;ch15-中断：创建内核任务期间禁止中断
         ;mov ecx,0x46
         ;call sys_routine_seg_sel:allocate_memory  ;ch15:为内核任务创建任务控制块TCB
         ...
         mov eax,cr3                        ;ch16
         mov dword [es:ecx+28],eax          ;ch16：分页：修改.登记CR3（PDBR）
         ...
         ;ch15:以下开始创建用户任务。创建任务控制块，这不是处理器的要求，而是我们自己为了方便设立的
         mov ecx,0x4a                       ;ch16
         call sys_routine_seg_sel:allocate_memory
         mov word [es:ecx+0x04],0           ;任务状态：就绪
         mov dword [es:ecx+0x46],0          ;ch16：分页：任务内可用于分配的初始线性地址
         ...
         mov dword [es:ecx+0x46],0          ;ch16：任务内可用于分配的初始线性地址
