...
;-------------------------------------------------------------------------------
         ...
         salt_item_len   equ $-salt_5               ;常量不占用空间，表中每个条目的长度，这里是262个字节，每个条目字节相同
         salt_items      equ ($-salt)/salt_item_len ;代表表中的条目数，这里是4个条目

         message_0        db  '  If you see this message,it means we are now '  ;ch15-中断：修改
         ...
         message_1        db  '  System wide CALL-GATE mounted and tested OK.'
                          db  0x0d,0x0a,0
...
load_relocate_program:                      ;加载并重定位用户程序
  ...
  .b4:
         ...
         ;ch14：创建2特权级堆栈
         mov ecx,0                          ;以4KB为单位的栈段界限值
         mov [es:esi+0x36],ecx              ;登记0特权级栈界限到TCB
         inc ecx
         shl ecx,12                         ;乘以4096，得到段大小
         push ecx
         call sys_routine_seg_sel:allocate_memory
         mov [es:esi+0x3a],ecx              ;登记0特权级堆栈基地址到TCB        
         mov eax,ecx
         ...
         mov dword [es:ecx+28],0            ;ch15：登记CR3（PDBR）
...
start:
  ...
  .b3:
         ...
         ;ch14:对门进行测试 
         mov ebx,message_1                  ;ch15-中断：修改
         call far [salt_1+256]              ;通过门显示信息(偏移量将被忽略) 
      
         ;ch15:为内核任务创建任务控制块TCB
