...
;-------------------------------------------------------------------------------
         ...
         cpu_brand  times 49 db 0           ;在192课中已经改成了49
...
load_relocate_program:                      ;加载并重定位用户程序
  ...
  .b1:
         ...
         mov ecx,[es:0x24]                  ;用户程序的SALT条目数，问题在这，偏移不再是edi+0x24，只有0x24
         mov edi,0x28                       ;用户程序内的SALT位于头部内0x2c处，不再是add了
  .b2: 
         push ecx
         push edi
  ...
  .b4:
         ...
         mov ax,[es:0x04]                   ;问题就出在这，返回地址又改了。不再是edi+0x04
...
start:
         ...
         jmp far [0x08]                      ;不再用call指令。控制权交给用户程序（入口点）
                                             ;堆栈可能切换 
